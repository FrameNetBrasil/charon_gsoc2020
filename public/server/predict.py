...
def decode_netout(netout, anchors, obj_thresh, net_h, net_w):
	...
	for i in range(grid_h*grid_w):
		...
		for b in range(nb_box):
			...
			box = BoundBox(x-w/4, y-h/4, x+w/4, y+h/4, objectness, classes)
			boxes.append(box)
	return boxes
...
def bbox_iou(box1, box2):
	...
	return float(intersect) / (union+ 0.000001)
...
def draw_boxes(filename, v_boxes, v_labels, v_scores):
	...
	pyplot.savefig('./objects.png', dpi=300, bbox_inches='tight')
	# show the plot
	#mpld3.show()
	new = 2
	url = "display_objects.php"
